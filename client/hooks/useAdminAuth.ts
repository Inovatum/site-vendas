      ...
      try {
        ...
        if (error) {
          ...
          const isMissingFn =
            ...
            /validate_admin_login/i.test(errorMsg);

          if (isMissingFn) {
            console.warn(
              "⚠️ Função SQL validate_admin_login ausente ou incompatível, usando autenticação simples como fallback",
            );
