...
import { ShoppingCart, Search, X, Plus, Minus, Trash2, Loader2, Grid3X3, LayoutGrid } from 'lucide-react';
...
export default function Index() {
  ...
  const generateWhatsAppMessage = () => {
    ...
    const message = `🛍️ *Pedido ${storeName}*\n\n` +
      cart.map(item =>
        `• ${item.name}\n` +
  ...
  return (
    <div key={customization.id + customization.button_color} className="min-h-screen" style={{ backgroundColor: customization.site_background_color }}>
        ...
        {!loading && !error && filteredProducts.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.map((product) => (
              <div
                key={product.id}
                className="rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden group"
                style={{ backgroundColor: customization.card_background_color, borderColor: customization.card_border_color }}
              >
               <div
  ...
  {product.image_2 && (
    <>
      <button
        onClick={(e) => {
          e.stopPropagation();
          setActiveImages((prev) => ({ ...prev, [product.id]: 0 }));
        }}
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          setActiveImages((prev) => ({ ...prev, [product.id]: 1 }));
        }}
    ...
    >
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md shadow-xl cart-panel" style={{ backgroundColor: customization.cart_color }}>
